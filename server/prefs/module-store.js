/**
 * One OpenVibe.Network user module Chat owns, per person: chat.preferences (./chat-preferences.js), and since
 * openvibe-contracts 0.41.0 chat.tts_defaults, chat.dm_settings and chat.presence_prefs (./stores.js).
 * Network stores each record, validates it against the namespace's schema, versions it (revision) and
 * announces every change (network.module.updated). createModuleStore(namespace, { label }) returns:
 *
 *   get(subject)            the record ({ data, revision, version }); {} and revision 0 when the person has
 *                           none. Cached per person for config.prefs.ttlMs; when Network cannot answer, a
 *                           cached copy is served (stale: true), else 503.
 *   peek(subject)           the cached data, or null, without asking Network (for synchronous paths).
 ...
 *                           writes nothing.
 *   handleEvent(envelope)   network.module.updated for this namespace newer than the cached copy drops it
 *                           at once (the Events consumer, server/events/consumer.js); without that
 *                           subscription the TTL bounds how long a change made outside Chat takes to show.
 *
...
function createModuleStore(NAMESPACE, { label = NAMESPACE, key = 'preferences' } = {}) {
    ...
    const view = (rec, extra = {}) => ({ namespace: NAMESPACE, version: rec.version, revision: rec.revision, [key]: { ...rec.data }, ...extra });
    ...
    function checkSubject(subject) {
        if (!enabled()) throw new PrefsError(503, 'prefs.disabled', `${label} are not configured on this server`);
    ...
    async function fetchRecord(subject) {
        ...
        if (r.status === 403) throw new PrefsError(503, 'prefs.not_granted', `Chat may not read ${NAMESPACE} (Network grant missing)`);
        throw unavailable(r);
    }
    ...
    function checkPatch(patch) {
        if (!patch || typeof patch !== 'object' || Array.isArray(patch)) throw new PrefsError(400, 'prefs.bad_request', `send { ${key}: { field: value } }`);
        const unknown = Object.keys(patch).filter((k) => !FIELDS.includes(k));
        if (unknown.length) throw new PrefsError(422, 'prefs.invalid', `unknown field${unknown.length > 1 ? 's' : ''}: ${unknown.join(', ')}`, unknown.map((k) => ({ path: `/${k}`, message: `not in ${NAMESPACE}` })));
    }
    ...
    async function update(subject, patch, { expectedRevision } = {}) {
        ...
        for (let i = 0; i < MAX_TRIES; i++) {
            ...
            if (expectedRevision !== undefined && cur.revision !== expectedRevision) {
                ...
                throw new PrefsError(412, 'prefs.revision_conflict', `the ${label} changed (revision ${cur.revision}, not ${expectedRevision}); read them again`);
            }
            const next = { ...cur.data };
            ...
            if (r.status === 412) {
                if (expectedRevision !== undefined) throw new PrefsError(412, 'prefs.revision_conflict', `the ${label} changed; read them again`);
        ...
        throw new PrefsError(409, 'prefs.busy', `the ${label} kept changing; try again`);
    }
    ...
    function peek(subject) { const hit = cache.get(subject); return hit ? { ...hit.data } : null; }

    const store = { NAMESPACE, FIELDS, get, peek, update, handleEvent, invalidate, _clear: () => cache.clear(), _cached: (subject) => cache.get(subject) || null };
    stores.push(store);
    return store;
}

/** Tests: an injected clock; clears every store's cache. */
function _configure({ now } = {}) { if (now) clock = now; for (const st of stores) st._clear(); }

module.exports = { PrefsError, enabled, createModuleStore, _configure };
