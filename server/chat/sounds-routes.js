/**
 * OpenVibe.Chat — Channel Sound Commands API (moved from OpenVibe.Live server/chat/sounds-routes.js, W6)
 *
 * Sounds are Chat's (channel_sounds); the files stay in the shared sounds directory
 * (SOUNDS_PATH = Live's data/sounds) because stored rows hold absolute paths into it and Live's
 * donation alerts read the alert sounds from there. Channels, their moderation settings (incl.
 * the alert-sound columns) and moderators are Live's and go through live-context.
 *
...
async function channelWithPolicy(userId) {
    const channel = await ctx.ensureChannelForUser(userId);
    if (channel) await ctx.ensurePolicy(channel.id);
    return channel;
}
...
router.get('/all/:streamId', async (req, res) => {
    try {
...
router.post('/', requireAuth, soundUpload.single('sound'), async (req, res) => {
    ...
    try {
        ...
        const channel = (await channelWithPolicy(channelOwnerId)) || (channelOwnerId === req.user.id ? await ctx.createChannel(req.user.id) : null);
        if (!channel) {
            cleanup();
...
router.patch('/command', requireAuth, async (req, res) => {
    try {
        ...
        if (!channelOwnerId) channelOwnerId = req.user.id;
        const channel = await channelWithPolicy(channelOwnerId);
...
router.post('/alert/:kind', requireAuth, soundUpload.single('sound'), async (req, res) => {
    ...
    try {
        ...
        const channel = (await channelWithPolicy(req.user.id)) || (await ctx.createChannel(req.user.id));
