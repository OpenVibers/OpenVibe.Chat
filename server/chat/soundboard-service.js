/**
 * OpenVibe.Chat — 101soundboards clips for !sb and pasted links (moved from OpenVibe.Live, W6).
 * The API key is a Live site setting, read through live-context; clips are cached in
 * CHAT_CACHE_DIR/soundboard-cache.
 */
...
const CACHE_DIR = path.join(config.cacheDir, 'soundboard-cache');
...
async function fetchSoundboardMeta(soundId) {
    const apiKey = String(ctx.getSetting('soundboard_101_api_key') || '').trim();
...
function isConfigured() {
    return !!String(ctx.getSetting('soundboard_101_api_key') || '').trim();
}
